# Done

| ID | Type | Title | Priority | Status |
|----|------|-------|----------|--------|
| 012 | feature | Refactor App.jsx into components | high | done |
| 014 | bug | The way App is managing states is not optimal (draggingFromLibrary lifted unnecessarily) | high | done |
| 015 | experimental | Done tasks in backlog should be moved to DONE.md | high | done |
| 013 | bug | the header is glitching | low | done |
| 001 | feature | Auth — PT vs Student roles (register/login) | high | done |
| 002 | feature | Backend + DB (Supabase: users, workouts, messages) | high | done |
| 001 | feature | Auth — PT vs Student roles (register/login) | high | done |
| 002 | feature | Backend + DB (Supabase: users, workouts, messages) | high | done |
| 012 | feature | Seeding script for dev environment | high | done |
| 001 | feature | Auth — PT vs Student roles (register/login) | high | done |
| 002 | feature | Backend + DB (Supabase: users, workouts, messages) | high | done |
| 012 | feature | Seeding script for dev environment | high | done |
| 003 | feature | Real-time messaging (Supabase Realtime) | high | done |
